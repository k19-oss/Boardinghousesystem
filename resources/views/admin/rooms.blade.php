...
{{-- ✅ FIX: Single unified alert div — auto-dismisses after 4s, no double popup --}}
@if(session('success'))
...
<div id="roomsGridContainer" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(220px, 1fr)); gap: 25px;">
    ...
    <div onclick="toggleRoomModal(true)" style="background: #f4f3f0; border: 2px dashed rgba(74, 54, 41, 0.3); border-radius: 20px; padding: 25px; text-align: center; display: flex; flex-direction: column; justify-content: center; align-items: center; min-height: 210px; cursor: pointer; transition: all 0.2s; box-sizing: border-box;"
        onmouseover="this.style.background='#eae8e3'; this.style.borderColor='rgba(74, 54, 41, 0.5)';"
        onmouseout="this.style.background='#f4f3f0'; this.style.borderColor='rgba(74, 54, 41, 0.3)';">
...
<div id="addRoomModal" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0, 0, 0, 0.4); z-index: 9999; align-items: center; justify-content: center;">
    <div class="card" style="background: white; border-radius: 20px; padding: 35px; width: 100%; max-width: 450px; box-shadow: 0 20px 50px rgba(0,0,0,0.15); border: 1px solid rgba(0,0,0,0.05); position: relative; margin: 20px;">
        ...
        <form action="{{ route('admin.rooms.store') }}" method="POST" style="display: flex; flex-direction: column; gap: 20px;">
            ...
            <div>
                <label style="display: block; font-size: 0.85rem; font-weight: 700; color: #57606f; margin-bottom: 8px; text-transform: uppercase; letter-spacing: 0.3px;">Room Classification</label>
                <select name="room_type" required style="width: 100%; padding: 12px 15px; border-radius: 10px; border: 1px solid #dcdde1; font-family: inherit; font-size: 0.95rem; box-sizing: border-box; background-color: white; color: #2f3542;">
                    <option value="Normal" selected>Normal Layout</option>
            ...
            <div style="display: flex; gap: 12px; margin-top: 10px;">
                <button type="button" onclick="toggleRoomModal(false)" style="flex: 1; background: #f1f2f6; color: #2f3542; padding: 12px; border-radius: 10px; border: none; font-weight: 700; font-size: 0.95rem; cursor: pointer;">Cancel</button>
                <button type="submit" style="flex: 1; background: #4A3629; color: white; padding: 12px; border-radius: 10px; border: none; font-weight: 700; font-size: 0.95rem; cursor: pointer;">Save Room</button>
            </div>
        </form>
    </div>
</div>
...
<div id="editRoomModal" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0, 0, 0, 0.4); z-index: 9999; align-items: center; justify-content: center;">
    <div class="card" style="background: white; border-radius: 20px; padding: 35px; width: 100%; max-width: 450px; box-shadow: 0 20px 50px rgba(0,0,0,0.15); border: 1px solid rgba(0,0,0,0.05); position: relative; margin: 20px;">
        ...
        <form id="editRoomForm" method="POST" style="display: flex; flex-direction: column; gap: 20px;">
            ...
            <div style="display: flex; gap: 12px; margin-top: 10px;">
                <button type="button" onclick="toggleEditModal(false)" style="flex: 1; background: #f1f2f6; color: #2f3542; padding: 12px; border-radius: 10px; border: none; font-weight: 700; font-size: 0.95rem; cursor: pointer;">Cancel</button>
                <button type="submit" style="flex: 1; background: #4A3629; color: white; padding: 12px; border-radius: 10px; border: none; font-weight: 700; font-size: 0.95rem; cursor: pointer;">Apply Changes</button>
            </div>
        </form>
    </div>
</div>
...
<script>
// ✅ FIX: Auto-dismiss session alerts after 4 seconds (prevents "stuck" double-looking alerts)
document.addEventListener('DOMContentLoaded', function () {
    const successAlert = document.getElementById('session-alert-success');
    const errorAlert   = document.getElementById('session-alert-error');
...
// ✅ FIX: Changed from 10s to 3s silent background refresh
setInterval(function () {
    fetch("{{ route('admin.rooms.data') }}")
        ...
        .then(data => {
            ...
            data.rooms.forEach(room => {
                ...
                const badgeStyle  = isPremium
                    ? 'background: rgba(255, 212, 59, 0.18); color: #ffd43b; border: 1px solid rgba(255, 212, 59, 0.25);'
                    : 'background: rgba(255, 255, 255, 0.12); color: #f1f2f6; border: 1px solid rgba(255, 255, 255, 0.15);';
                ...
                // room.price from the API is a numeric, not a string — no replace needed
                const rawPrice      = parseFloat(room.price) || 0;
                const formattedPrice = rawPrice.toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 0 });
        ...
        .catch(error => console.error('Room sync heartbeat error:', error));
}, 3000); // ✅ 3 seconds
</script>
@endsection